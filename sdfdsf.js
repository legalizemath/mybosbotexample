...
// let it request resetting node from another process to fix connections (by creating a file)
...
const updateFees = async () => {
  ...
  for (const peer of peers) {
    ...
    if (isIncreasing) {
      ...
      // prettier-ignore
      const feeIncreaseLine = `${getDate()} ${ca(peer.alias).padEnd(30)} ${ppmOld.toFixed(0).padStart(5)} -> ${ppmNewTrunc.toFixed(0).padEnd(6)} ppm ${ppmNewString} ${flowString.padStart(15)} ${flowOutDaysString} days  ${localSats}|${remoteSats}  max htlc: ${byChannelPretty.padStart(11)} ${outflowString}  ⚠️ ${warnings}`
